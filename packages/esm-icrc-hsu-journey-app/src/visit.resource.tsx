...
export function useVisits(patientUuid: string) {
  const customRepresentation =
    'custom:(uuid,encounters:(id,uuid,form:(uuid,display),encounterDatetime,' +
    'orders:full,' +
...
export function useVisit(visitUuid: string | null): any {
  const customRepresentation =
    'custom:(uuid,encounters:(id,uuid,form:(uuid,display),encounterDatetime,' +
    'orders:full,' +
    'obs:(uuid,concept:(uuid,display,conceptClass:(uuid,display)),' +
  ...
  const { data, error, isValidating } = useSWR<{ data: { results: Array<Visit> } }, Error>(
    visitUuid ? `/ws/rest/v1/visit/${visitUuid}?v=${customRepresentation}` : null,
    openmrsFetch,
  );

  return {
    visits: data && !isValidating ? data.data : null,
    isError: error,
    isLoading: !data && !error && !!visitUuid,
    isValidating,
  };
}
...
export function usePastVisits(patientUuid: string) {
  const customRepresentation =
    ...
    'visitType:(uuid,name,display),attributes:(uuid,display,value),location:(uuid,name,display),startDatetime,stopDatetime,' +
    'stopDatetime)';
